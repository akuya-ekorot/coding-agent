...
export const runCommandWithStreaming = createTool({
  id: "runCommandWithStreaming",
  description:
    "Run a shell command with real-time output streaming in the e2b sandbox",
  inputSchema: z.object({
    sandboxId: z.string().describe(
      "The sandboxId for the sandbox to run the command in",
    ),
  ...
  outputSchema: z.object({
    success: z.boolean().describe("Whether the command started successfully"),
    exitCode: z.number().optional().describe(
      "The exit code of the command (if not background)",
    ),
    stdoutLines: z.array(z.string()).describe("Array of stdout lines received"),
    stderrLines: z.array(z.string()).describe("Array of stderr lines received"),
    command: z.string().describe("The command that was executed"),
    executionTime: z.number().optional().describe(
      "How long the command took to execute in milliseconds (if not background)",
    ),
  }).or(z.object({
    error: z.string().describe("The error from a failed command execution"),
  })),
  execute: async ({ context }) => {
    try {
      ...
      const executionTime = context.background
        ? undefined
        : Date.now() - startTime;

      return {
        success: true,
        exitCode: context.background ? undefined : result.exitCode,
        stdoutLines,
        stderrLines,
        command: context.command,
        executionTime,
